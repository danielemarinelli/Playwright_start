...
const rows:Locator=table.locator("tr"); //returns all the rows including header (LOCATOR CHAINING)
...
for(let text of secondRowTexts)
{
 ...
 // 4) Read all data from the table (excluding header)
  ...
  const allRowData=await rows.all();  // get all rows locators   //all() returns array of locators
